...
Partial Public Class LA4page
    Inherits System.Web.UI.Page
    Private EquipmentID As String = "LA4"
    ...
    Private LinacFlag As String = "StateLA4"
    Private appstate As String = "LogOnLA4"
    Private suspstate As String = "SuspendedLA4"
    Private actionstate As String = "ActionStateLA4"
    Private repairstate As String = "rppTabLA4"
    Private failstate As String = "FailStateLA4"
    Private clinicalstate As String = "ClinicalOnLA4"
    Private treatmentstate As String = "TreatmentLA4"
    Private activetabstate As String = "ActTabLA4"
    ...
    Private faultstate As String = "faultstateLA4"
    Private linacloaded As String = "LA4loaded"
    Private returnclinical As String = "ReturnClinicalLA4"
    Private technicalstate As String = "techstateLA4"
    Private recover As String = Nothing
    Private lsctrl As LinacStatusuc
    Private RegistrationState As String = "regstateLA4"
    ...
    Protected Sub Page_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init
        ...
        End If
        AddHandler LA4page.NoApprove, AddressOf ClinicalApprovedEvent
    ...
    Public Sub LaunchTab()
        ...
        If Not IsPostBack Then
            ...
            If Reload = True Then
                ...
                Dim rucontrol As UserControl = tcl.ActiveTab.FindControl(runupcontrolId)
                Dim preccontrol As UserControl = tcl.ActiveTab.FindControl(preclincontrolID)
                Dim clincontrol As UserControl = tcl.ActiveTab.FindControl(ClinicalUserControlID)
                Dim plancontrol As UserControl = tcl.ActiveTab.FindControl(PlannedMaintenanceControlID)
                Dim repcontrol As UserControl = tcl.ActiveTab.FindControl(repcontrolId)
                Dim webcontrol As UserControl = tcl.ActiveTab.FindControl(webusercontrol21ID)
                Dim writecontrol As UserControl = tcl.ActiveTab.FindControl(writedatacontrolID)
                Dim physicscontrol As UserControl = tcl.ActiveTab.FindControl(physicscontrolID)
                Dim emergencycontrol As UserControl = tcl.ActiveTab.FindControl(emergencycontrolID)
                Dim trainingcontrol As UserControl = tcl.ActiveTab.FindControl(trainingcontrolID)
    ...
    Public Event MyEventLA4 As System.EventHandler
    ...
    Protected Sub ReportFault_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles ReportFault.Click
        ...
        End If
        'Response.Redirect("faultPage.aspx?val=LA4&Tabindex=" & Tabindex & "&commentbox=" & textcomment)
    ...
    '    'Response.Redirect("RegisterUser.aspx?val=LA4&Tabindex=" & Tabindex)
    'End Sub
    ...
    Public Shared Sub CloseMessage()
        ...
        Try
            Dim lastState As String
            lastState = DavesCode.Reuse.GetLastState("LA4", 0)
            Select Case lastState
                Case "Repair", "Fault"
                    DavesCode.Reuse.SetStatus("No User", "Repair", 5, 7, "LA4", 0)
                Case Else
                    DavesCode.Reuse.SetStatus("No User", "Linac Unauthorised", 5, 7, "LA4", 0)
            End Select
        Finally
    ...
    Sub WriteRecovery()
        ...
        Select Case Activity
            ...
            Case 2
                DavesCode.Reuse.CommitPreClin(EquipmentID, Userinfo, Comment, False, False, False, breakdown)
